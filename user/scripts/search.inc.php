...
if (strcmp($table, "clients") == 0) {
    ...
    searchMyAppointments($con, $name, $myid);
}elseif (strcmp($table, "myAppCanceled") == 0) {
    searchMyCAppointments($con, $name, $myid);
}  else {
    echo "1";
}
...
function searchMyCAppointments($con, $name, $myid)
{
    if (empty($name)) {
        echo $error = "1";
    } else {
        ...
        <tr>
                        ...
                        <?php
                                                $sql1 = "SELECT clients.name,clients.surname,appointments.shdesc,appointments.date,appointments.startTime,appointments.endTime ,appointments.status from clients,appointments where appointments.userId='$myid' and appointments.clientId=clients.id and clients.name like '%$name%' and appointments.status='2' ORDER BY clients.name ASC";
                        $res1 = $con->query($sql1);
                        if ($res1->num_rows > 0) {
                            $i = 1;
                            while ($data = $res1->fetch_assoc()) {
...
function searchMyAppointments($con, $name, $myid)
{
    if (empty($name)) {
        echo $error = "1";
    } else {
      ...
      <tr>
                        ...
                        <?php
                        $sql1 = "SELECT clients.name,clients.surname,appointments.shdesc,appointments.date,appointments.startTime,appointments.endTime ,appointments.status from clients,appointments where appointments.userId='$myid' and clients.name like '%$name%' and appointments.clientId=clients.id ORDER BY clients.name ASC";
                        $res1 = $con->query($sql1);
                        if ($res1->num_rows > 0) {
                            $i = 1;
                            while ($data = $res1->fetch_assoc()) {
