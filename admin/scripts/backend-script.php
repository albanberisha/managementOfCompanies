...
if(!empty($_GET['id']) && !empty($_GET['name']) && $_GET['name']=='color_setting')
{
  ...
  $updateData=update_data($data,$tableName, $id);
   if($updateData){
    echo "<span class='success'>Ngjyrat u perditesuan me sukses</span>";
...
if( !empty($_GET['id']) && !empty($_GET['tableName']) && $_GET['role']=='admin'){
   ...
   $tableName="users";
   $companyId = "";
	$query = "SELECT companyId from users where id='$id'";
   ...
   if($role['status']==1)
   {
    ...
    $query2="SELECT id,status FROM users WHERE companyId=$companyId and status!=2 and privilege!=2";
    ...
    while($row=mysqli_fetch_array($res2))
    {
      ...
      $return ="fas fa-user-alt-slash iconRole";
      $updateData=update_data($data,$tableName, $row['id']);
      if($updateData)
      {
       echo $return;
      }else{
...
if(!empty($_GET['name']) && $_GET['name']=='change_password'){
    ...
    $new2=md5($_POST['new_password2']);
    if(empty($_POST['password']))
    {
      echo "<span class='fail'>".$old."</span>";
      exit;
    }
    if(!empty($old) && !empty($new) && !empty($new2))
    {
        $query="SELECT password FROM users WHERE password='$old' and id='$myid'";
      ...
      echo "<span class='fail'>Të gjitha fushat duhet të plotësohen.</span>";
    }
  }
...
if(!empty($_GET['id']) && !empty($_GET['name']) && $_GET['name']=='admin_profile'){
  ...
  echo "<span class='fail'>Admin Profile field is empty2</span>";
}
...
if(!empty($_GET['id']) && !empty($_GET['name']) && $_GET['name']=='website_setting')
{
  ...
  {
    echo "<span class='fail'>Emri nuk mund te jete i zbrazet.</span>";
  }else{
    $uploadTo="../../images/";   
    ...
    $uploadingError="";
    if(!in_array($websiteLogoType, $allowedFileType) && !empty($websiteLogo)){
      $uploadingError="Vetem 'jpg','jpeg','png','gif' formatet lejohen.";
    }
    ...
    if(empty($uploadingError)){
     ...
     $updateData=update_data($data,"companydetails", "1");
      if($updateData){
       echo "<span class='success'>Te dhenat u perditesuan me sukses</span>";
       echo '<script>parent.window.location.reload(true);</script>';
      }else{
       echo "<span class='fail'>Error!.. check your query</span>";
      }
      ...
      echo "<span class='fail'>".$uploadingError. "</span>";
    }
  }
  //echo "<span class='fail'>".$websiteLogoPath."</span>";
}

/* 
  $name=$_GET['website_title'];
  $initials=$_GET['website_name'];
  $uploadTo="../../images/";   
  $websiteLogo=$_FILES['website_logo']['name'];
  $tempWebsiteLogo=$_FILES['website_logo']['tmp_name'];
  $state=$_GET['state'];
  ...
  echo "<span class='fail'>".$name."</span>";
...
if(!empty($_GET['deleteId']) && !empty($_GET['deleteData']))
{
   ...
   if(strcmp($tableName,'companies')==0)
   {
    $query = "SELECT * from users where companyId='$id' and status!='2'";
    $result = mysqli_query($con, $query);
    $row = mysqli_fetch_array($result);
    if ($row > 0) {
      ob_clean();
      ...
      $deleteData=delete_data2($tableName, $id);
    
    if($deleteData){
      echo "<span class='success'>Kompania eshte fshire</span>";
    }else{
      echo  "<span class='fail'>Error...Check your query</span>";
    }
   }
    
   }elseif(strcmp($tableName,'users')==0)
   {

      $query = "SELECT * from users where companyId IN (SELECT companyId from users Where id='$id') and status!='2' and privilege='2'";
      $result = mysqli_query($con, $query);
      if(mysqli_num_rows($result)>1)
      {
        ...
        if($deleteData){
          echo "<span class='success'>Administratori eshte fshire</span>";
        }else{
          echo  "<span class='fail'>Error...Check your query</span>";
        }
      }else{
        ...
        $deleteData=delete_dataAll($tableName, $compid);
        if($deleteData){
          echo "<span class='success'>Administratori dhe te gjithe shfrytezuesit e tij jane fshire</span>";
        }else{
          echo  "<span class='fail'>Error...Check your query</span>";
        }
...
// ======= cancel appointments from database ============//

if(!empty($_GET['cancelId']) && !empty($_GET['cancelData']))
{    
   $id= legal_input($_GET['cancelId']);
   $deleteData=legal_input($_GET['cancelData']);
   $tableName= $deleteData;
  
   $deleteData=delete_data($tableName, $id);
    if($deleteData){
      echo "<span class='success'>Termini eshte anuluar</span>";
    }else{
      echo  "<span class='fail'>Error...Check your query</span>";
    }
...
function delete_data2($tableName, $id){
  global $db;
...
function delete_dataAll($tableName, $compid){
  global $db;
  $query="UPDATE ".$tableName." SET status='2' WHERE companyId=".$compid;
  $result= $db->query($query);
  if($result){
     return true;
  }else{
...
function delete_data($tableName, $id){
  global $db;
  $query="UPDATE ".$tableName." SET status='2' WHERE id=".$id;
  $result= $db->query($query);
  if($result){
     return true;
  }else{
