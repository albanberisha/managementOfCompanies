...
<div class="content-box">
    <?php if (!empty($_GET['view'])) { ?>
        ...
        $uerResp=$viewData['registeredBy'];
        $regdate=$viewData['regDate'];
        $update=$viewData['updateDate'];
        $updateid=$viewData['updatedBy'];

        $query = "SELECT * FROM users WHERE id=$uerResp";
        $res = $con->query($query);
        $viewData = $res->fetch_assoc();
        if (empty($viewData)) {
            echo "Asnje e dhene per tu shfaqur!";
        ...
        $usersurname = $viewData['surname'];
        $useremail = $viewData['email'];
        $usersusername = $viewData['username'];
        ?>
        <div class="row">
            <div class="col">
            </div>
        ...
        <div class="row">
				<div class="col-sm-12">
					<h6>Regjistruar me: <strong><?php echo $regdate; ?> </strong></h6>
                    <?php
                    if(!empty($update))
                    {
                        $query2 = "SELECT * FROM users WHERE id=$updateid";
                        $res2 = $con->query($query2);
                        $viewData2 = $res2->fetch_assoc();
                        if (empty($viewData2)) {
                            echo "Asnje e dhene per tu shfaqur!";
                            exit;
                        }
                        ?>
                        <h6>Hera e fundit e përditësimit: <strong><?php echo $update; ?> </strong></h6>
                        <h6>Perditesuar nga: <strong><?php echo $viewData2['name']; ?> </strong></h6>
                        <?php
                    }
                    ?>
    ...
    ?>
        ...
        <div class="row">
            <div class="col">
            <a href="javascript:void(0);" onclick="showAddAppointmentForm();" class="btn btn-secondary btn-back content-link"><i class="fas fa-plus" aria-hidden="true"></i> Shto një tërmin</a>
            </div>
        </div>
        <div class="row">
            <div class="col" style="text-align: center;">
